...
- **Full-screen key.** Press **F** anywhere (except in a text field) to toggle
  the Fullscreen API — `toggleFullscreen()` on the global keydown handler. The
  index blurb mentions it.
- **QR code (files only, not on the site).** `qr.svg` / `qr.png` at the repo
  root, generated by `scripts/make_qr.py` (needs `segno`) from
  `https://jorgegallud-boop.github.io/FSA/`. Jorge uses them in slides / handouts
